...
Translated (deterministic structural transforms): `rev`, `palindrome`, `fast N`/`density N`/`hurry N` (hurry is notation-equivalent to fast), `ply N`, `degradeBy X`/`unDegradeBy X` (grey shading, same as `?`), `every N f`, `whenmod A B f`, `repeatCycles N`, `iter N`/`iter' N` (exact when the rotation lands on an element boundary), `rot N` (rotates values, rhythm stays), `euclid K N`/`euclidInv K N`/`euclidOff K N O`, `struct "t f [t t] f"` (the boolean pattern becomes the notated structure), `jux f`/`juxBy X f`/`superimpose f` (simultaneous voices in one staff), `always f`/`never f`, and a global `all $ f` line (applies to every flagged line). Chains also work typed directly into the playground or a live-line row.

Deliberately **not** translated on the staff (the graphic score and Played-staff mode show their real result): random functions (`sometimes`, `irand`, `shuffle`, `scramble`, `fadeIn/Out`, `someCycles`), sample-slicing (`chop`, `striate`), whole-line `slow N` (needs cross-barline tie slicing — future work), time-slicing functions (`off`, `within`, `linger`, `trunc`, `chunk`, `swing`, `stut`, `echo`, `brak`, `inside`/`outside`), boolean family beyond struct (`mask`, `sew`, `stitch`, `inv`, `euclidBool` — straightforward follow-ups on struct's machinery), and audio params (`cut`, `gain`, `pan`…) which don't change rhythm structure.
...
- **Note numbers follow Tidal exactly**: in the default pitch mode, `note "0 4 7"` works — 0 = c5 = middle C (MIDI 60), negatives go down (`-12` = c4). Add/subtract 60 to convert to/from raw MIDI (a separate "Raw MIDI" input mode accepts 60-style numbers directly).
